...
def get_rotation_matrix(yaw_deg, pitch_deg, roll_deg):
    ...
    # Rotation matrices (Assuming ZYX order: Yaw(Z) -> Pitch(Y) -> Roll(X) or similar)
    # Note: The exact rotation order for RealityCapture/v360 mapping might need adjustment.
    ...
    # Rotation around Y axis (Roll - or whatever axis v360 uses for roll)
    # v360 docs: yaw=up, pitch=right, roll=forward.
    # If we assume standard camera: Forward=Z, Up=Y, Right=X.
    # Then Yaw=Y, Pitch=X, Roll=Z.
    
    # Let's try constructing based on standard Y-up, Z-forward convention.
    # Yaw (around Y)
    Ry = np.array([
    ...
    # Combined Rotation R = Ry(yaw) * Rx(pitch) * Rz(roll)
    # Order of multiplication depends on intrinsic vs extrinsic.
    # Let's assume R = Ry @ Rx @ R_roll
    R = Ry @ Rx @ R_roll
